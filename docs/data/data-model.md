...
- `outbox_events`
  ...
  - Includes retry state (`status`, `retry_count`, `next_attempt_at`, `last_error`) and claim lease fields (`claimed_at`, `claim_until`).
...
- Additional uniqueness `(account_id, idempotency_key, created_at)` supports replay/audit without globally locking the key forever.
